...
def verify_book_chapter(book, chapter, bible_version = 'akjv'):
    ...
    try:
        requesting = requests.get('json_api_url'+str(book)+str(chapter)+'&version='+bible_version)
...
def get_message(message, bible_version = 'akjv'):
    ...
    if message[-1].isnumeric():
        requesting = requests.get('json_api_url'+str(message)+'&version='+bible_version)
        text = requesting.text[1:-2]
...
def get_next_chapter(present_chapter, bible_version = 'akjv'):    
    ...
    try:
        ...
        requesting = requests.get('json_api_url'+str(next_chapter)+'&version='+bible_version)
        text = requesting.text[1:-2]
